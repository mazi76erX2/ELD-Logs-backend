...
class ELDLogSheet(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    eld_log = models.OneToOneField(
        ELDLog, on_delete=models.CASCADE, related_name="log_sheet"
    )
    log_sheet_data = models.JSONField(null=True, blank=True)
    log_image = models.TextField(null=True, blank=True)  # Base64 encoded image
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Log Sheet for {self.eld_log}"
